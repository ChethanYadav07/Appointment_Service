...
//	int countTotalAppointments(@Param("id") int id);
	...
	@Query("SELECT a FROM Appointment a WHERE a.vetId = :vetId")
	List<Appointment> findByVetId(@Param(value ="vetId") long vetId);

	@Query("SELECT a FROM Appointment a WHERE a.petParentId = :petParentId")
	List<Appointment> findByPetparentId(@Param(value ="petParentId") long petParentId);
	
	@Query(value = "SELECT a FROM Appointment a WHERE a.vetId = :vetId AND a.appointmentStatus = :status")
